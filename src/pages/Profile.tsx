...
export default function Profile() {
  ...
  const loadData = async () => {
    try {
      ...
      // 1. Cargar Perfil + Hobbies del Usuario
      ...
      // 2. Cargar Catálogo Completo de Hobbies
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Actualizar Preferencias en 'profiles'
  ...
  // LÓGICA ACTUALIZADA: Selección Única
  const togglePreference = (value: string) => {
    if (!profile) return;
    
    const isSelected = profile.preferences.includes(value);
    
    // Si ya está seleccionado, lo quitamos (array vacío) -> Esto deshabilitará el botón
    // Si no, reemplazamos todo el array con este nuevo valor (solo 1) -> Esto habilita el botón
  ...
  // Debe haber exactamente 1 preferencia seleccionada
          ...
          <CardContent className="space-y-6 pt-6 bg-white">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                {!isPreferencesValid && (
                  <span className="text-[10px] text-destructive flex items-center gap-1 animate-pulse">
